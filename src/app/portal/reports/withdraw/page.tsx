...
import { ArrowUpFromLine, Clock, CheckCircle, XCircle, Search } from "lucide-react";

interface Withdrawal { id: string; amount: number; status: string; paymentMethod: string; reference: string; adminComment: string | null; createdAt: string; }
...
export default function WithdrawReportPage() {
  ...
  const fetchData = (f?: string, t?: string) => {
    ...
    let url = `/api/portal/withdrawals?userId=${userId}`;
    if (f) url += `&from=${f}`;
    if (t) url += `&to=${t}`;
  ...
  const approved = data.filter((d) => d.status === "approved" || d.status === "completed");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-400 to-red-600 flex items-center justify-center shadow-md shadow-red-500/20"><ArrowUpFromLine className="w-5 h-5 text-white" /></div>
      <div><h1 className="text-2xl font-bold text-gray-900">Withdraw Report</h1><p className="text-sm text-gray-500">View your withdrawal history</p></div></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 p-5"><p className="text-xs font-semibold text-gray-500 uppercase">Total Withdrawn</p><p className="text-2xl font-bold text-red-600 mt-1">${totalAmount.toLocaleString()}</p></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Withdrawals</h3></div>
        {data.length === 0 ? <div className="p-10 text-center"><p className="text-sm text-gray-400">No transactions yet!</p></div> : (
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-gray-50/80 border-b border-gray-100">
            ...
            {data.map((w) => (<tr key={w.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
