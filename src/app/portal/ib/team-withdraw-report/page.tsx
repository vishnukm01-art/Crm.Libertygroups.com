...
export default function TeamWithdrawReportPage() {
  ...
  const fetchData = (f?: string, t?: string) => {
    ...
    let url = `/api/portal/team-reports?userId=${userId}&type=withdraw`;
    if (f) url += `&from=${f}`;
    if (t) url += `&to=${t}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div><h1 className="text-2xl font-bold text-gray-900">My Team Withdraw Report</h1><p className="text-sm text-gray-500">Withdrawals from your referred clients</p></div></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 p-5"><p className="text-xs font-semibold text-gray-500 uppercase">Total</p><p className="text-2xl font-bold text-red-600 mt-1">${data.summary.total.toLocaleString()}</p></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100"><h3 className="font-semibold text-gray-900">Withdraw Report</h3></div>
