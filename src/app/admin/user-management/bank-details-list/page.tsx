...
export default function BankDetailsListPage() {
  ...
  useEffect(() => {
    fetch("/api/bank-details?status=all")
      .then((r) => (r.ok ? r.json() : []))
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "approved":
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Approved</span>;
      case "rejected":
  ...
  return (
    <PageShell title="Bank Details List" description="All submitted bank details and their statuses" icon={CreditCard}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Bank Name</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Account Number</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">No bank details found</td></tr>
              ) : (
                paginated.map((b) => (
