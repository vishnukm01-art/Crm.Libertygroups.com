...
interface User {
  id: string; name: string; email: string; mt5Account: string | null; mt5Group: string | null; leverage: string | null; status: string;
}

export default function ActiveTradersPage() {
  ...
  useEffect(() => {
    fetch("/api/users").then(r => r.ok ? r.json() : []).then(data => {
      setUsers(data.filter((u: User) => u.status === "active" && u.mt5Account));
  ...
  return (
    <PageShell title="Active Traders" description="Users actively trading on MT5" icon={TrendingUp}>
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden animate-fade-in-up">
        <div className="p-4 border-b border-gray-100 flex items-center gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search active traders..."
          ...
          <span className="text-sm text-gray-400 font-medium">{filtered.length} traders</span>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">Loading...</td></tr> :
               filtered.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">No active traders found</td></tr> :
               filtered.map(u => (
                <tr key={u.id} className="table-row-hover border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-4 py-3 text-gray-600">{u.mt5Group || "--"}</td>
                  <td className="px-4 py-3 text-gray-600">{u.leverage || "--"}</td>
                  <td className="px-4 py-3"><span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">Active</span></td>
                </tr>
              ))}
            </tbody>
          </table>
