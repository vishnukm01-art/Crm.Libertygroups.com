...
interface User {
  id: string; name: string; email: string; mt5Account: string | null; status: string; role: string; createdAt: string;
}

export default function NonFTDUsersPage() {
  ...
  useEffect(() => {
    fetch("/api/users").then(r => r.ok ? r.json() : []).then(data => {
      setUsers(data.filter((u: User) => !u.mt5Account && u.role === "client"));
  ...
  return (
    <PageShell title="Non FTD Users" description="Users without first-time deposit" icon={UserX}>
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden animate-fade-in-up">
        <div className="p-4 border-b border-gray-100 flex items-center gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search non-FTD users..."
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400">Loading...</td></tr> :
               filtered.length === 0 ? <tr><td colSpan={5} className="px-4 py-12 text-center text-gray-400">No non-FTD users found</td></tr> :
               filtered.map(u => (
                <tr key={u.id} className="table-row-hover border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-4 py-3"><button className="text-xs text-sky-600 hover:text-sky-700 font-medium">Create MT5</button></td>
                </tr>
              ))}
            </tbody>
          </table>
