...
export default function DepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/mt5/deposit", {
      ...
      setSuccess(`Deposit of $${form.amount} processed successfully!`);
      setForm({ userId: "", amount: "", comment: "" });
    } catch { setError("An error occurred"); } finally { setLoading(false); }
  };

  return (
    <PageShell title="Deposit" description="Process deposits to MT5 accounts" icon={ArrowDownToLine}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input type="text" value={form.comment} onChange={e => setForm({...form, comment: e.target.value})}
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-sky-300 focus:ring-2 focus:ring-sky-100 focus:outline-none transition-all" placeholder="Deposit comment (optional)" />
          </div>
          <button type="submit" disabled={loading} className="btn-primary disabled:opacity-50">{loading ? "Processing..." : "Process Deposit"}</button>
        </form>
      </div>
    </PageShell>
  );
