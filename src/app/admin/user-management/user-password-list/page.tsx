...
interface User {
  id: string; name: string; email: string; mt5Account: string | null; status: string;
}

export default function UserPasswordListPage() {
  ...
  return (
    <PageShell title="User Password List" description="Manage user passwords" icon={KeyRound}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">Loading...</td></tr> :
               filtered.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">No users found</td></tr> :
               filtered.map(u => (
                <tr key={u.id} className="table-row-hover border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/admin/user-management/change-password?user=${u.id}`}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-500 hover:bg-sky-600 text-white text-xs font-medium rounded-xl transition-colors">
