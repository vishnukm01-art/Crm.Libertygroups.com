...
import { Wallet, CheckCircle, AlertCircle, DollarSign, User, MessageSquare } from "lucide-react";
...
export default function WalletDepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/transactions", {
        ...
        body: JSON.stringify({ type: "wallet_deposit", userId, amount: Number(amount), comment }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to process wallet deposit"); return; }
      setSuccess(`Successfully deposited $${Number(amount).toFixed(2)} to ${selectedClient?.name || "client"}'s wallet`);
  ...
  return (
    <PageShell title="Wallet Deposit" description="Add funds to client wallet" icon={Wallet}>
        ...
        <div className="lg:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="bg-gradient-to-r from-sky-500 to-blue-600 px-6 py-4">
              <h3 className="text-white font-bold text-base">Wallet Deposit</h3>
              <p className="text-sky-100 text-xs mt-0.5">Add funds directly to client&apos;s wallet balance</p>
            </div>
            <div className="p-6 space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <select value={userId} onChange={(e) => setUserId(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100 bg-white appearance-none">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Enter reason for wallet deposit..." rows={3} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100 resize-none" />
                </div>
              </div>
              <button type="submit" disabled={loading} className="w-full px-4 py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl hover:from-sky-600 hover:to-blue-700 transition-all font-semibold text-sm disabled:opacity-50 shadow-lg shadow-sky-500/20 hover:shadow-xl hover:shadow-sky-500/30 active:scale-[0.98] flex items-center justify-center gap-2">
                {loading ? (
                  <><div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> Processing...</>
                ) : (
                  <><Wallet className="w-4 h-4" /> Deposit to Wallet</>
                )}
              </button>
            </div>
          </form>
        ...
        <div className="space-y-4">
          <div className="bg-gradient-to-br from-sky-50 to-blue-50 rounded-2xl border border-sky-100 p-5">
            <h4 className="font-bold text-sky-900 text-sm mb-3 flex items-center gap-2">
              <AlertCircle className="w-4 h-4 text-sky-600" /> How It Works
            </h4>
            <ul className="space-y-2 text-xs text-sky-800">
              <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-sky-200 text-sky-700 flex items-center justify-center text-xs font-bold shrink-0">1</span> Select the client account</li>
              <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-sky-200 text-sky-700 flex items-center justify-center text-xs font-bold shrink-0">2</span> Enter the deposit amount</li>
              <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-sky-200 text-sky-700 flex items-center justify-center text-xs font-bold shrink-0">3</span> Add a comment and submit</li>
              <li className="flex items-start gap-2"><span className="w-5 h-5 rounded-full bg-sky-200 text-sky-700 flex items-center justify-center text-xs font-bold shrink-0">4</span> Wallet balance updates instantly</li>
            </ul>
          </div>
          {selectedClient && (
