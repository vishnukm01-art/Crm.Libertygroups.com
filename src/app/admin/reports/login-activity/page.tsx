...
import { UserCheck, Calendar, Download, Search } from "lucide-react";

interface LoginRecord {
  id: string; name: string; email: string; ipAddress: string;
  city: string; state: string; country: string; date: string;
}
...
export default function LoginActivityPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/login-activity?${params.toString()}`);
  ...
  const exportExcel = () => {
    const headers = ["ID", "Name", "Email", "IP Address", "City", "State", "Country", "Date"];
    const rows = filtered.map((r, i) => [i + 1, r.name, r.email, r.ipAddress, r.city, r.state, r.country, r.date]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "login-activity.csv"; a.click();
  };

  return (
    <PageShell title="Login Activity" description="User login activity report" icon={UserCheck}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["ID", "Name", "Email", "IP Address", "City", "State", "Country", "Date"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id || idx} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td className="px-3 py-3 text-gray-500 font-mono text-xs">{r.ipAddress}</td>
                  <td className="px-3 py-3 text-gray-700">{r.city || "-"}</td>
                  <td className="px-3 py-3 text-gray-700">{r.state || "-"}</td>
