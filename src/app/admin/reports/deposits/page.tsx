...
import { ArrowDownToLine, Calendar, Download, Search } from "lucide-react";

interface DepositRecord {
  id: string; userId: string; userName: string; userEmail: string;
  mt5Id: string; amount: number; paymentMethod: string | null;
...
export default function DepositReportPage() {
  ...
  const exportExcel = () => {
    const headers = ["ID", "Name/Email", "MT5 ID", "Amount", "Payment Method", "Note", "Comment", "Status", "Date", "Marketing Name", "Approved/Rejected By"];
    const rows = filtered.map((r, i) => [i + 1, `${r.userName} (${r.userEmail})`, r.mt5Id || "-", r.amount, r.paymentMethod || "-", r.note || "", r.comment || "", r.status, new Date(r.createdAt).toLocaleDateString(), r.marketingName || "-", r.approvedBy || "-"]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "deposit-report.csv"; a.click();
  };

  return (
    <PageShell title="Deposit Report" description="Deposit transaction reports" icon={ArrowDownToLine}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["ID", "Name / Email", "MT5 ID", "Amount", "Payment Method", "Note", "Comment", "Status", "Date", "Marketing Name", "Approved/Rejected By"].map((h) => (
