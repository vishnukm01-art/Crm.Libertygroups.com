...
import { ArrowUpFromLine, Calendar, Download, Search } from "lucide-react";

interface WithdrawRecord {
  id: string; userId: string; userName: string; userEmail: string;
  mt5Id: string; amount: number; withdrawTo: string; paymentMethod: string | null;
  note: string; comment: string; status: string; createdAt: string;
  marketingName: string; approvedBy: string;
}
...
export default function WithdrawalReportPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ type: "withdraw" });
  ...
  const exportExcel = () => {
    const headers = ["ID", "Name/Email", "MT5 ID", "Amount", "Withdraw To", "Payment Method", "Note", "Comment", "Status", "Date", "Marketing Name", "Approved/Rejected By"];
    const rows = filtered.map((r, i) => [i + 1, `${r.userName} (${r.userEmail})`, r.mt5Id || "-", r.amount, r.withdrawTo || "-", r.paymentMethod || "-", r.note || "", r.comment || "", r.status, new Date(r.createdAt).toLocaleDateString(), r.marketingName || "-", r.approvedBy || "-"]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "withdrawal-report.csv"; a.click();
  };

  return (
    <PageShell title="Withdraw Report" description="Withdrawal transaction reports" icon={ArrowUpFromLine}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["ID", "Name / Email", "MT5 ID", "Amount", "Withdraw To", "Payment Method", "Note", "Comment", "Status", "Date", "Marketing Name", "Approved/Rejected By"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td className="px-3 py-3 text-gray-700">{r.mt5Id || "-"}</td>
                  <td className="px-3 py-3 text-red-600 font-medium">${Number(r.amount).toFixed(2)}</td>
