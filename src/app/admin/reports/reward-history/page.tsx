...
interface RewardRecord {
  id: string; userName: string; userEmail: string; rewardName: string;
  volume: number; withdrawType: string; amount: number; status: string;
  date: string; marketingName: string; approvedBy: string;
}

const statusBadge = (status: string) => {
  const map: Record<string, string> = {
    claimed: "bg-emerald-100 text-emerald-700", approved: "bg-emerald-100 text-emerald-700",
...
export default function RewardHistoryReportPage() {
  ...
  const exportExcel = () => {
    const headers = ["ID", "Name/Email", "Reward Name", "Volume", "Withdraw Type", "Amount", "Status", "Date", "Marketing Name", "Approved/Rejected By"];
    const rows = filtered.map((r, i) => [i + 1, `${r.userName} (${r.userEmail})`, r.rewardName, r.volume || 0, r.withdrawType || "-", r.amount, r.status, r.date, r.marketingName || "-", r.approvedBy || "-"]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "reward-history-report.csv"; a.click();
  };

  return (
    <PageShell title="Reward History" description="Reward claims and history report" icon={Trophy}>
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <button onClick={() => { setDateFrom(""); setDateTo(""); setStatusFilter(""); }} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm hover:bg-gray-200 font-medium">Clear</button>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["ID", "Name / Email", "Reward Name", "Volume", "Withdraw Type", "Amount", "Status", "Date", "Marketing Name", "Approved/Rejected By"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} className="px-4 py-12 text-center"><div className="h-6 w-6 animate-spin rounded-full border-3 border-sky-500 border-t-transparent mx-auto" /></td></tr>
              ) : paginated.length === 0 ? (
                ...
                <tr key={r.id || idx} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td className="px-3 py-3 text-gray-700">{r.rewardName}</td>
                  <td className="px-3 py-3 text-gray-700">{r.volume || 0}</td>
                  <td className="px-3 py-3 text-gray-700">{r.withdrawType || "-"}</td>
                  <td className="px-3 py-3 text-emerald-600 font-medium">${Number(r.amount).toFixed(2)}</td>
                  <td className="px-3 py-3">
