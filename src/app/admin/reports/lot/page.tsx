...
interface LotRecord {
  login: string; date: string; ticket: string; symbol: string;
  price: number; profit: number; volume: number; action: string;
}

export default function LotReportPage() {
  const [data, setData] = useState<LotRecord[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/lot?${params.toString()}`);
      if (res.ok) {
        const result = await res.json();
        setData(result.data || []);
        setTotalLot(result.totalLot || 0);
      }
  ...
  const exportExcel = () => {
    const headers = ["Login", "Date", "Ticket", "Symbol", "Price", "Profit", "Volume", "Action"];
    const rows = filtered.map((r) => [r.login, r.date, r.ticket, r.symbol, r.price, r.profit, r.volume, r.action]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "lot-report.csv"; a.click();
  };

  return (
    <PageShell title="Lot Report" description="Trading lot volume report" icon={BarChart3}>
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="ml-auto bg-sky-50 border border-sky-100 rounded-xl px-4 py-2">
            <span className="text-xs text-sky-600">Total Lot</span>
            <p className="text-lg font-bold text-sky-700">{totalLot.toFixed(2)}</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["Login", "Date", "Ticket", "Symbol", "Price", "Profit", "Volume", "Action"].map((h) => (
