...
import { ArrowLeftRight, Calendar, Download, Search } from "lucide-react";

interface TransferRecord {
  id: string; userName: string; userEmail: string; fromAccount: string;
  toAccount: string; amount: number; date: string; marketingName: string;
}
...
export default function InternalTransferReportPage() {
  ...
  const exportExcel = () => {
    const headers = ["ID", "Name/Email", "From Account", "To Account", "Amount", "Date", "Marketing Name"];
    const rows = filtered.map((r, i) => [i + 1, `${r.userName} (${r.userEmail})`, r.fromAccount || "-", r.toAccount || "-", r.amount, r.date, r.marketingName || "-"]);
    const csv = [headers.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "internal-transfer-report.csv"; a.click();
  };

  return (
    <PageShell title="Internal Transfer Report" description="Internal fund transfer report" icon={ArrowLeftRight}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                {["ID", "Name / Email", "From Account", "To Account", "Amount", "Date", "Marketing Name"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-12 text-center"><div className="h-6 w-6 animate-spin rounded-full border-3 border-sky-500 border-t-transparent mx-auto" /></td></tr>
              ) : paginated.length === 0 ? (
                ...
                <tr key={r.id || idx} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td className="px-3 py-3 text-gray-700">{r.fromAccount || "-"}</td>
                  <td className="px-3 py-3 text-gray-700">{r.toAccount || "-"}</td>
                  <td className="px-3 py-3 text-sky-600 font-medium">${Number(r.amount).toFixed(2)}</td>
